...
public class MultiThreadServer extends Application
{
    ...
    private Map<Integer, CustomSocket> clients = new HashMap<>();
    ...
    public void start(Stage primaryStage)
    {
        ...
        new Thread(new Runnable() {
            ...
            public void run()
            {
                try
                {
                    ServerClientSocket serverSocket = new ServerClientSocket(8000);

                    while (true)
                    {
                        CustomSocket socket = serverSocket.accepts();
    ...
    class NewClient implements Runnable
    {
        ...
        public NewClient(CustomSocket socket){
            this.socket = socket;
        }
            ...
            try
            {
                ...
                while (true)
                {
                    ...
                    for (Iterator<Integer> iter = clients.keySet().iterator(); iter.hasNext();)
                    {
                        ...
                        CustomSocket client = clients.get(key);

                        outputToClient = new PrintWriter(client.getOutputStream());

                        outputToClient.println(client.getPcName() + " - " + textClient);
